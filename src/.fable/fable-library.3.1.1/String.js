...
export function compare(...args) {
  switch (args.length) {
    ...
    case 5:
      return cmp(
        args[0].substr(args[1], args[4]),
        args[2].substr(args[3], args[4]),
        false
      );
    case 6:
      return cmp(
...
export function interpolate(input, values) {
  ...
  return input.replace(
    interpolateRegExp,
    (_, prefix, flags, padLength, precision, format) => {
...
function formatOnce(str2, rep, padRef) {
  return str2.replace(
    fsFormatRegExp,
    (match, prefix, flags, padLength, precision, format) => {
...
export function format(str, ...args) {
  ...
  return str.replace(
    formatRegExp,
    (_, idx, padLength, format, precision, pattern) => {
      let rep = args[idx];
      if (isNumeric(rep)) {
        precision = precision == null ? null : parseInt(precision, 10);
        switch (format) {
          ...
          case 'D':
            rep =
              precision != null
                ? padLeft(String(rep), precision, '0')
                : String(rep);
            break;
          case 'x':
          case 'X':
            ...
            if (pattern) {
              ...
              rep = pattern.replace(
                ...
                (_, intPart, decimalPart) => {
                  ...
                  rep = toFixed(
                    rep,
                    decimalPart != null ? decimalPart.length - 1 : 0
                  );
                  return padLeft(
                    rep,
                    (intPart || '').length -
                      sign.length +
                      (decimalPart != null ? decimalPart.length : 0),
                    '0'
                  );
                }
              );
...
export function fromBase64String(b64Encoded) {
  const binary =
    typeof atob === 'function' ? atob(b64Encoded) : notSupported('atob');
...
export function remove(str, startIndex, count) {
  ...
  return (
    ...
    (typeof count === 'number' ? str.substr(startIndex + count) : '')
  );
}
