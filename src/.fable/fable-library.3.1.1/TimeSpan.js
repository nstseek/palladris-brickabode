...
export function toString(ts, format = 'c', _provider) {
  ...
  const sign = ts < 0 ? '-' : '';
  return `${sign}${
    d === 0 && (format === 'c' || format === 'g')
      ? ''
      : format === 'c'
      ? d + '.'
      : d + ':'
  }${format === 'g' ? h : padWithZeros(h, 2)}:${padWithZeros(
    m,
    2
  )}:${padWithZeros(s, 2)}${
    ms === 0 && (format === 'c' || format === 'g')
      ? ''
      : format === 'g'
      ? '.' + padWithZeros(ms, 3)
...
export function parse(str) {
  ...
  if (firstDot === -1 && firstColon === -1) {
    ...
    if (isNaN(d)) {
      throw new Error(
        `String '${str}' was not recognized as a valid TimeSpan.`
      );
    } else {
  ...
  if (firstColon > 0) {
    ...
    const r = /^(-?)((\d+)\.)?(?:0*)([0-9]|0[0-9]|1[0-9]|2[0-3]):(?:0*)([0-5][0-9]|[0-9])(:(?:0*)([0-5][0-9]|[0-9]))?\.?(\d+)?$/.exec(
      str
    );
