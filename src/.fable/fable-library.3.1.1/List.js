...
import {
  transpose as transpose_1,
  pairwise as pairwise_1,
  collect as collect_1,
  scanBack as scanBack_1,
  scan as scan_1,
  foldBack2 as foldBack2_1,
  fold2 as fold2_1,
  fold as fold_1,
  map as map_1
} from './Seq.js';
...
import {
  splitInto as splitInto_1,
  chunkBySize as chunkBySize_1,
  permute as permute_1,
  findIndexBack as findIndexBack_1,
  tryFindIndexBack as tryFindIndexBack_1
} from './Array.js';
      ...
      loop: while (true) {
        ...
        if (matchValue[0].tail != null) {
          if (matchValue[1].tail != null) {
            const matchValue_1 =
              comparer(matchValue[0].head, matchValue[1].head) | 0;
            if (matchValue_1 === 0) {
...
export function concat(lists) {
  return reverse(
    fold_1(
      (state, xs) => fold((acc, x) => new List_1(x, acc), state, xs),
      new List_1(),
      lists
    )
  );
...
export function mapIndexed(f, xs) {
  return reverse(
    foldIndexed((i, acc, x) => new List_1(f(i, x), acc), new List_1(), xs)
  );
}
...
export function map2(f, xs, ys) {
  return reverse(
    fold2((acc, x, y) => new List_1(f(x, y), acc), new List_1(), xs, ys)
  );
}
...
export function mapIndexed2(f, xs, ys) {
  return reverse(
    foldIndexed2(
      (i, acc, x, y) => new List_1(f(i, x, y), acc),
      new List_1(),
      xs,
      ys
    )
...
export function map3(f, xs, ys, zs) {
  return reverse(
    fold3(
      (acc, x, y, z) => new List_1(f(x, y, z), acc),
      new List_1(),
      xs,
      ys,
      zs
...
export function mapIndexed3(f, xs, ys, zs) {
  return reverse(
    foldIndexed3(
      (i, acc, x, y, z) => new List_1(f(i, x, y, z), acc),
      new List_1(),
      xs,
      ys,
      zs
...
export function filter(f, xs) {
  return reverse(
    fold((acc, x) => (f(x) ? new List_1(x, acc) : acc), new List_1(), xs)
  );
}
...
export function partition(f, xs) {
  return fold(
    uncurry(2, (tupledArg) => {
      ...
      return (x) =>
        f(x) ? [new List_1(x, lacc), racc] : [lacc, new List_1(x, racc)];
    }),
    [new List_1(), new List_1()],
    reverse(xs)
  );
}
...
export function unzip(xs) {
  return foldBack(
    (tupledArg, tupledArg_1) => [
      new List_1(tupledArg[0], tupledArg_1[0]),
      new List_1(tupledArg[1], tupledArg_1[1])
    ],
    xs,
    [new List_1(), new List_1()]
  );
...
export function unzip3(xs) {
  return foldBack(
    (tupledArg, tupledArg_1) => [
      new List_1(tupledArg[0], tupledArg_1[0]),
      new List_1(tupledArg[1], tupledArg_1[1]),
      new List_1(tupledArg[2], tupledArg_1[2])
    ],
    xs,
    [new List_1(), new List_1(), new List_1()]
  );
}
...
export function sort(xs, comparer) {
  ...
  return ofArray(
    ((xs_1 = Array.from(xs)),
    (xs_1.sort((x, y) => comparer.Compare(x, y)), xs_1))
  );
}
...
export function sortBy(projection, xs, comparer) {
  ...
  return ofArray(
    ((xs_1 = Array.from(xs)),
    (xs_1.sort((x, y) => comparer.Compare(projection(x), projection(y))), xs_1))
  );
}
...
export function sortDescending(xs, comparer) {
  ...
  return ofArray(
    ((xs_1 = Array.from(xs)),
    (xs_1.sort((x, y) => comparer.Compare(x, y) * -1), xs_1))
  );
}
...
export function sortWith(comparer, xs) {
  ...
  return ofArray(
    ((comparer_1 = comparer),
    ((xs_1 = Array.from(xs)), (xs_1.sort(comparer_1), xs_1)))
  );
}
...
export function maxBy(projection, xs, comparer) {
  return reduce(
    (x, y) => (comparer.Compare(projection(y), projection(x)) > 0 ? y : x),
    xs
  );
}
...
export function minBy(projection, xs, comparer) {
  return reduce(
    (x, y) => (comparer.Compare(projection(y), projection(x)) > 0 ? x : y),
    xs
  );
}
...
export function average(xs, averager) {
  return averager.DivideByInt(
    fold((acc, x) => averager.Add(acc, x), averager.GetZero(), xs),
    length(xs)
  );
}
...
export function averageBy(f, xs, averager) {
  return averager.DivideByInt(
    fold((acc, x) => averager.Add(acc, f(x)), averager.GetZero(), xs),
    length(xs)
  );
}
...
export function splitAt(i, xs) {
  ...
  if (matchValue[0] < 0) {
    ...
    switch (pattern_matching_result) {
      ...
      case 1: {
        return [
          new List_1(matchValue[1].head, new List_1()),
          matchValue[1].tail
        ];
      }
      case 2: {
