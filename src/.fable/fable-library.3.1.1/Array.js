...
export function mapIndexed3(f, source1, source2, source3, cons) {
  if (
    source1.length !== source2.length ? true : source2.length !== source3.length
  ) {
    throw new Error('Arrays had different lengths');
  }
  const result = new (cons || Array)(source1.length);
...
export function groupBy(projection, array, eq) {
  ...
  return map(
    (key_1) => [key_1, Array.from(getItemFromDict(dict, key_1))],
    keys,
    null
  );
}
...
export function choose(chooser, array, cons) {
  return map(
    (x_1) => value_2(chooser(x_1)),
    array.filter((x) => chooser(x) != null),
    cons
  );
}
...
export function foldIndexed(folder, state, array) {
  return array.reduce(
    (delegateArg0, delegateArg1, delegateArg2) =>
      folder(delegateArg2, delegateArg0, delegateArg1),
    state
  );
}
...
export function zip3(array1, array2, array3) {
  if (
    array1.length !== array2.length ? true : array2.length !== array3.length
  ) {
    differentLengths();
  }
  const result = new Array(array1.length);
...
export function foldBackIndexed(folder, array, state) {
  return array.reduceRight(
    (delegateArg0, delegateArg1, delegateArg2) =>
      folder(delegateArg2, delegateArg1, delegateArg0),
    state
  );
}
...
export function foldBack(folder, array, state) {
  return array.reduceRight(
    (delegateArg0, delegateArg1) => folder(delegateArg1, delegateArg0),
    state
  );
}
...
export function foldBack2(f, array1, array2, state) {
  return foldBackIndexed2(
    (_arg1, x, y, acc) => f(x, y, acc),
    array1,
    array2,
    state
  );
...
export function forAll2(predicate, array1, array2) {
  return fold2(
    (acc, x, y) => (acc ? predicate(x, y) : false),
    true,
    array1,
    array2
  );
...
export function splitInto(chunks, array) {
  ...
  if (array.length === 0) {
    ...
    for (let i = 0; i <= chunks_1 - 1; i++) {
      const chunkSize =
        (i < chunksWithExtraItem ? minChunkSize + 1 : minChunkSize) | 0;
      let slice;
      const start_1 =
        (i * minChunkSize + min_1(comparePrimitives, chunksWithExtraItem, i)) |
        0;
      slice = array.slice(start_1, start_1 + chunkSize);
      const value = result.push(slice);
      void value;
